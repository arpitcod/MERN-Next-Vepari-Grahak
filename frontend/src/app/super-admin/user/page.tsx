      ...
      
      {
        filteredUsers?.length === 0 && searchQuery ? (
          ...
          filteredUsers?.map((user, index) => (
            <div
              key={index}
              className="border border-gray-400 rounded-2xl shadow-md bg-white hover:shadow-xl transition duration-300 p-4 flex justify-between items-center space-y-2"
            >
              <div className="flex flex-col gap-1">
                <div className="text-gray-800 text-lg flex gap-3">
                  <strong className="font-medium text-indigo-600">Username:</strong>
                  <p>{user.username}</p>
                </div>
              ...
              <div>
                <button
                  className="flex items-center justify-center gap-0.5 h-10 w-10 border bg-red-500 text-indigo-50 hover:bg-red-600 transition-all rounded-full cursor-pointer"
